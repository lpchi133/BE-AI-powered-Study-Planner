...
export class TasksService {
  ...
  async updateTimeTask(userId: number, id: number, start_date: string, date: string) {
    ...
    const start_time = task.dateTimeSet ? task.dateTimeSet.toString().split('T')[1].slice(0, 5) : '00:00';  // Lấy thời gian từ dateTimeSet
    const time = task.dueDateTime ? task.dueDateTime.toString().split('T')[1].slice(0, 5) : '00:00';  // Lấy thời gian từ dueDateTime
